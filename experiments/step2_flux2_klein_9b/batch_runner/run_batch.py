"""
run_batch.py — batch FLUX-2-Klein-9B Step 2 swap.

Self-contained. Does NOT depend on or modify the parent run.py or trace_html.py.
Imports only the unchanged step_2_flux2_klein.py for the fal API call, and reads
config.yaml from the parent folder so single-run and batch use identical params.

Re-runs Step 2 with FLUX-2-Klein-9B Edit across ALL scenarios from an existing
...
Output goes to:
    experiments/step2_flux2_klein_9b/batch_runner/outputs/<timestamp>_batch/
        ...
        <scenario_id>/
            ...
            05_step2_flux2_klein_final.jpg    (NEW)
            05_step2_flux2_klein_meta.json    (NEW)
...
from experiments.step2_flux2_klein_9b.batch_runner import trace_html_batch as batch_trace
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Batch-run FLUX-2-Klein-9B Step 2 against an entire Plan A run."
    )
    parser.add_argument(
        "--reuse-run-root",
        type=Path,
    ...
    # Note on speed: distilled FLUX-2-Klein-9B uses 4-step inference, so per-call
    # latency is significantly faster than Qwen (~5-10s vs ~30s). Batch should
    # finish in ~3-5 min for 30 scenarios instead of ~15.
    avg_call_seconds = 8

    print(f"[batch] reuse-run-root:  {root}")
    print(f"[batch] scenarios found: {n}")
    print(f"[batch] estimated cost:  ${estimated_cost:.2f}")
    print(f"[batch] est. wall time:  ~{n * avg_call_seconds // 60} min "
          f"({n * avg_call_seconds}s sequential @ ~{avg_call_seconds}s each)")
    ...
    try:
        for i, scenario_dir in enumerate(scenario_dirs, start=1):
            ...
            if status == "success":
                fmeta = record.get("step_2_flux2_meta") or {}
                print(
                    f"[batch]   OK  {fmeta.get('elapsed_seconds', 0):.1f}s "
                    f"${fmeta.get('cost_usd', 0):.3f}"
                )
            else:
    ...
    actual_cost = sum(
        (r.get("step_2_flux2_meta") or {}).get("cost_usd", 0)
        for r in records
    ...
    for r in records:
        light_records.append({
            ...
            "flux2_elapsed": (r.get("step_2_flux2_meta") or {}).get("elapsed_seconds"),
            "flux2_cost": (r.get("step_2_flux2_meta") or {}).get("cost_usd"),
            "flux2_seed": (r.get("step_2_flux2_meta") or {}).get("seed"),
