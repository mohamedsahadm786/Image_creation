"""
trace_html_batch.py — visual reports for the batch runner.
...
Self-contained — does NOT import or modify the parent trace_html.py.
  ...
  - write_overview_html(batch_dir, records, summary): grid of all scenarios with
                                                      Nano Banana / Qwen side-by-side

Path math note: chain.html sits at
  experiments/step2_qwen_edit_2511/batch_runner/outputs/<ts>_batch/<sid>/chain.html
...
def write_chain_html(out_dir: Path, record: dict) -> None:
    ...
    model_label = record.get("model_label", "Qwen")
    ...
    panels = [
        ...
        (f"Step 2 — {model_label} (NEW)",
         "Experiment output", "05_step2_qwen_final.jpg"),
    ]
  ...
  <img src="{html.escape(src)}" alt="{html.escape(caption)}"
    ...
    qwen_time = _fmt_seconds(qwen_meta)
    nb_cost = _fmt_cost(nb_meta)
    qwen_cost = _fmt_cost(qwen_meta)
    nb_seed = nb_meta.get("seed", "—")
    qwen_seed = qwen_meta.get("seed", "—")
  ...
  <strong>Qwen call failed:</strong> {html.escape(str(error_message))}
</div>"""
...
<head>
<meta charset="UTF-8">
<title>{html.escape(sc_id)} — Qwen Step 2 batch</title>
<style>
  *{{box-sizing:border-box}}
  ...
  .badge-experiment{{background:#DBEAFE;color:#1E3A8A;margin-left:0;margin-right:6px}}
...
<div class="compare-strip">
  ...
  <div class="col">
    ...
    <div class="col-row">endpoint: <strong>{html.escape(qwen_meta.get('endpoint', 'fal-ai/qwen-image-edit-2511'))}</strong></div>
    <div class="col-row">elapsed: <strong>{qwen_time}</strong></div>
    <div class="col-row">cost: <strong>{qwen_cost}</strong></div>
    <div class="col-row">seed: <strong>{html.escape(str(qwen_seed))}</strong></div>
  </div>
</div>
...
def write_overview_html(batch_dir: Path, records: list, summary: dict) -> None:
    """
    Render an overview.html at the batch root.
    Each card shows Nano Banana (left) next to Qwen (right) with the scenario
    label, status, and timing. Click a card to open its 4-panel chain.html.
    """
    n = len(records)
    ...
    model_label = summary.get("model_label", "Qwen")
    ...
    for r in records:
        ...
        qmeta = r.get("step_2_qwen_meta") or {}
        elapsed_s = qmeta.get("elapsed_seconds")
...
<div class="card">
  ...
  <a href="{html.escape(chain_path)}" class="card-img-link">
    <div class="card-imgs">
      ...
      <div class="card-img-half">
        <div class="card-img-tag tag-qwen">Qwen</div>
        <img src="{html.escape(qwen_img)}" alt="qwen"
             onerror="this.outerHTML='<div class=card-empty>—</div>'">
      </div>
    </div>
  </a>
...
<head>
<meta charset="UTF-8">
<title>Qwen Step 2 batch — {html.escape(timestamp)}</title>
<style>
  *{{box-sizing:border-box}}
  body{{font-family:-apple-system,BlinkMacSystemFont,system-ui,sans-serif;
  ...
  .card-img-tag.tag-qwen{{background:rgba(30,58,138,.85)}}
...
<body>
<header>
  <h1>Qwen-Image-Edit-2511 — Step 2 swap experiment{html.escape(title_suffix)}</h1>
  <div class="meta">
...
<div class="legend">
  Each card shows <strong>Nano Banana (left)</strong> next to <strong>Qwen (right)</strong>. Click a card to open the full 4-panel chain.html.
</div>
<div class="grid">
{''.join(cards)}
</div>
