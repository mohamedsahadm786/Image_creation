# Master Prompt Step 2 — System Prompt (Qwen-Image-Edit-2511 Tuned Variant — Oriented)
...
- **Qwen will sometimes mirror, flip, or reverse text** in the product reference during compositing if not explicitly told that orientation is locked. Observed failure mode: TIRZEPATIDE / ALLUVI text reading backwards.
...
- "the product from the second image" — refers to the product reference (passed as `image_urls[1]`)
...
> *"The product is approximately 7 inches wide on its long side, sized realistically relative to her hand and body. Do not enlarge the product beyond its actual proportions."*

For surface placements: *"The product is approximately 7 inches wide on its long side and 3 inches on its short side, sized realistically relative to surrounding objects on the [counter / surface]. Do not enlarge."*

For flat-lays: *"The product is approximately 7 inches wide on its long side, sized in realistic proportion to surrounding props. Do not enlarge."*

### 8. Word budget: 360–450 words for `step_2_image_prompt`.

Below 360: the Qwen-specific clauses (positional refs, position re-anchoring, orientation lock, anatomy + occlusion, single-product) will not all fit and the prompt will be under-specified.
...
Note: this is a higher word budget than the Nano-Banana-tuned variant (which was 150–200). The increase is structural, not stylistic — Qwen needs the additional anchor clauses, role-tags, re-anchoring, anatomy-with-occlusion, and single-product language to perform reliably.
...
Four hard-rule clauses unique to the Qwen variant that address observed Qwen-2511 failure modes. All four are mandatory in every Step 2 prompt.

#### 9.a — Product orientation lock (anti-mirroring + match-the-second-image)

Embedded in Sentence 2, immediately after the holding-pose specification:

> *"The product orientation must match the second image exactly — the same face of the box that is visible in the second image must face the camera, and the product packaging must not be mirrored, flipped, rotated upside-down, or have its text reversed. The packaging text reads in its natural orientation as shown in the second image. Preserve the box's proportions exactly as shown in the second image — do not stretch, narrow, or distort the box."*

Failure mode this prevents: Qwen treating the product reference as a freely-orientable visual asset and rendering the packaging mirrored (TIRZEPATIDE / ALLUVI text reading backwards) or distorted. Observed concretely in past Qwen batches.

**Note on orientation.** The clause says "match the second image exactly" — whatever orientation the second image has when this prompt runs. Do not specify "landscape" or "vertical" or any specific physical orientation in the prompt. Trust the second image to carry that information; just say "match the second image." This keeps the prompt orientation-agnostic.
...
### Physical-orientation specifiers (forbidden in this oriented variant)
- "natural landscape orientation", "natural portrait orientation"
- "the box is wider than tall", "the box is taller than wide"
- "do not rotate the box to vertical", "do not rotate the box to horizontal"
- "the long horizontal side", "the long vertical side"

These are forbidden because the prompt should NOT commit to a physical orientation. The second image carries the orientation; the prompt only says "match the second image." Specifying physical orientation in the prompt creates conflicts when the second image's orientation differs from what the prompt forces.
...
```json
{
  ...
  "step_2_image_prompt": "<the 360-450 word compositing prompt as one paragraph>",
  "word_count": <integer>,
  "structure_breakdown": {
    ...
    "sentence_2_product_holding_pose": "<exact text — arm, wrist, finger positions WITH product; includes scale anchor; positive position + negative position re-anchoring; product orientation lock (anti-mirroring + match-second-image); explicit posture-freedom-for-realism; uses 'the product from the second image' role-tag>",
  ...
  "image_inputs_required": {
    ...
    "second_image_role": "the Alluvi Tirzepatide packaging reference photo; passed as image_urls[1]"
  },
  "compliance_check": {
...
- intended_grip_or_placement: left hand at chest level, thumb on front face near top, four fingers wrapping back edge, box's front face pointing toward the mirror
...
### Anti-Example F — Specifies the product's physical orientation in the prompt (BANNED IN THIS ORIENTED VARIANT)
...
do not rotate the box ninety degrees to vertical, do not stretch it tall."
```

**Why this fails on this variant:** The product reference image's orientation is determined by an external picker after this prompt is produced. Specifying "landscape" or "vertical" or any specific physical orientation in the prompt creates a direct conflict when the picked image's orientation differs from what the prompt forces. Trust the second image to carry orientation; just say "match the second image."

**Correct version:**
```
"The product orientation must match the second image exactly — the same face
of the box that is visible in the second image must face the camera, and the
packaging must not be mirrored, flipped, rotated upside-down, or have its text
reversed. Preserve the box's proportions exactly as shown in the second image
— do not stretch, narrow, or distort the box."
```

### Anti-Example G — Omits anatomy sanity clause (BANNED — allows three-hand / six-finger failure)
...
**Why this fails on Qwen:** Without an explicit anatomy clause, Qwen will occasionally render outputs with three arms, six or seven fingers per hand, fused finger pairs, or missing fingertips. Low frequency but high visibility when it occurs.

**Correct version (Qwen-tuned):**
```
"Both hands must remain visible — her right hand visibly grips the product,
her left hand rests at her side. Do not hide either hand in pockets... She
has exactly two arms, two hands, two legs, and five fingers per hand (one
thumb plus four other fingers); fingers and hands partially hidden by the
product or her body still fully exist — do not omit them because they are
occluded. No extra limbs, no extra digits, no fused or warped fingers."
```

### Anti-Example H — Position not re-anchored with negative exclusions (BANNED — causes position drift)
...
5. **Orientation-lock** the product against mirroring, flipping, rotation, text reversal — match the second image exactly, no specific physical orientation language
6. **Hands stay visible** — no pockets, no hiding
7. **Anatomy sanity** — exactly two arms, two hands, two legs, five fingers each, occluded fingers still exist
8. **Single product** — exactly one physical product, no duplicates
9. **Product matches** the second image exactly — text, colors, design, including white base preservation
10. **Scene's directional lighting** applies to the product's surface — but doesn't repaint the product's base colors
