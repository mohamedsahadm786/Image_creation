...
### 8. Word budget: 320–410 words for `step_2_image_prompt`.

Below 320: the Qwen-specific clauses (positional refs, position re-anchoring, orientation lock + landscape orientation, anatomy sanity) will not all fit and the prompt will be under-specified.
Above 410: the prompt becomes diffuse and Qwen's semantic encoder weights distract clauses against each other. Documented Qwen behavior — signal-to-noise ratio drops past this length.

Note: this is a higher word budget than the Nano-Banana-tuned variant (which was 150–200). The increase is structural, not stylistic — Qwen needs the additional anchor clauses, role-tags, re-anchoring, and landscape-orientation language to perform reliably.
...
[Sentence 4: PRODUCT FIDELITY (vs second image) + LIGHTING DIRECTION
  + WHITE BASE PRESERVATION, 50–70 words]
```
...
```json
{
  ...
  "structure_breakdown": {
    ...
    "sentence_4_product_fidelity_and_lighting": "<exact text — product matches second image exactly, lighting direction matches scene, product white base preserved>"
  },
  "fal_qwen_params": {
...
```json
{
  ...
  "structure_breakdown": {
    ...
    "sentence_4_product_fidelity_and_lighting": "The Alluvi product packaging must match the product in the second image exactly — every text element, color, graphic, and certification badge as shown. Preserve the packaging's natural white base color, only apply the scene's directional lighting on top, do not tint the white to match the scene's warm tones. Match the lighting direction of the persona scene: soft warm natural daylight from the window on her left, with a soft shadow falling toward her right."
  },
  "fal_qwen_params": {
    "image_size": {"width": 768, "height": 1344},
    "num_images": 1,
...
**Word budget for `step_2_image_prompt`: 320–410 words target, 410–440 acceptable for complex scenarios with mirror reflections, dual lighting, or multi-prop placements.**
