...
def build_step_2_prompt_qwen(scenario: dict, step_1_output: dict) -> dict:
    ...
    user_message = "\n".join(
        [
            ...
            "Sentence 2 MUST include the product orientation lock clause",
            "  (anti-mirroring + anti-flipping + anti-rotation + anti-text-reversal +",
            "   the box stays in its NATURAL LANDSCAPE ORIENTATION as shown in the second image,",
            "   long horizontal side parallel to the camera plane, NOT rotated to vertical).",
            ...
            "   duplicates'; for mirror-reflection scenarios add 'the mirror reflection counts",
            "   as the same product, not a second one').",
