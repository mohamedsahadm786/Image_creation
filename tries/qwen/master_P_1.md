...
### 8. Word budget: 280–380 words for `step_2_image_prompt`.

Below 280: the new Qwen-specific clauses (positional refs, position re-anchoring, orientation lock, anatomy sanity) will not all fit and the prompt will be under-specified.
Above 380: the prompt becomes diffuse and Qwen's semantic encoder weights distract clauses against each other.

Note: this is a higher word budget than the Nano-Banana-tuned variant (which was 150–200). The increase is structural, not stylistic — Qwen needs the additional anchor clauses, role-tags, and re-anchoring language to perform reliably. This is in line with Qwen's documented preference for specific, detailed prompts that name what stays and what changes.
...
**Word budget for `step_2_image_prompt`: 280–380 words target, 380–420 acceptable for complex scenarios with mirror reflections, dual lighting, or multi-prop placements.**
